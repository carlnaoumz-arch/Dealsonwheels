...
export default function ExteriorTour({photos,title}:{photos:string[];title:string}){
  ...
  function stop(event:PointerEvent<HTMLDivElement>){if(drag.current?.id!==event.pointerId)return;drag.current=null;setDragging(false);seek(Math.round(target.current));if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId)}
  ...
  return <div className="exterior-tour">
    <div className={`photo-stage tour-stage exterior-stage ${dragging?'is-dragging':''}`} tabIndex={0} role="group" aria-label={`${title} exterior photo tour`} aria-describedby={instructions} aria-busy={loading}
      onPointerDown={start} onPointerMove={move} onPointerUp={stop} onPointerCancel={stop} onLostPointerCapture={()=>{if(drag.current)seek(Math.round(target.current));drag.current=null;setDragging(false)}}
    ...
    <div className="gallery-controls exterior-controls"><button aria-label="Previous exterior view" disabled={loading||ready.length<2} onClick={()=>seek(Math.round(target.current)-1)}><ArrowLeft size={21}/></button><button className="tour-play" disabled={loading||ready.length<2||hidden} onClick={()=>{if(playing)seek(Math.round(current.current));else setPlaying(true)}} aria-label={playing?'Pause exterior tour':'Play exterior tour'}>{playing?<Pause size={17}/>:<Play size={17}/>}<span>{playing?'Pause tour':'Play tour'}</span></button><button aria-label="Next exterior view" disabled={loading||ready.length<2} onClick={()=>seek(Math.round(target.current)+1)}><ArrowRight size={21}/></button></div>
    <div className="tour-controls"><p id={instructions}><MoveHorizontal size={19}/><span>Drag or swipe to explore the exterior. Use Play for a hands-free tour.</span></p><div className="tour-scrubber"><span id={sliderLabel}>EXTERIOR VIEWS</span><Slider aria-labelledby={sliderLabel} disabled={loading||ready.length<2} min={0} max={Math.max(1,ready.length-.001)} step={.01} value={[scrub]} onValueChange={value=>seek(Array.isArray(value)?value[0]:value)} onValueCommitted={value=>seek(Math.min(ready.length-1,Math.round(Array.isArray(value)?value[0]:value)))}/></div><p className="tour-disclosure">Original exterior photographs with blended transitions. Available angles vary; this is not a continuous 3D scan.</p>{failed&&<p className="tour-disclosure">Some photographs could not load. <button className="text-link" onClick={()=>setRetry(n=>n+1)}>Retry</button></p>}</div>
  </div>;
}
